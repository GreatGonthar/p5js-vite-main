	...
	for (let j = 0; j < DIM; j++) {
		for (let i = 0; i < DIM; i++) {
			...
			if (j > 0) {
				...
				if (upCell && upCell.options) {
					try {
						const valid = upCell.options.flatMap(
							(opt) => rules[opt][2]
						); // DOWN rule
			...
			if (i < DIM - 1) {
				...
				if (rightCell && rightCell.options) {
					try {
			...
			if (j < DIM - 1) {
				...
				if (downCell && downCell.options) {
					try {
						const valid = downCell.options.flatMap(
							(opt) => rules[opt][0]
						); // UP rule
						checkValid(options, valid);
					} catch (err) {
			...
			if (i > 0) {
				...
				if (leftCell && leftCell.options) {
					try {
						const valid = leftCell.options.flatMap(
							(opt) => rules[opt][1]
						); // RIGHT rule
			...
			if (i === 0 && chunks.length > 0) {
				...
				const rightmostCell = lastChunk[(j + 1) * DIM - 1];
				if (rightmostCell && rightmostCell.options) {
					const valid = rightmostCell.options.flatMap(
						(opt) => rules[opt][1]
					); // RIGHT rule
					checkValid(options, valid);
				}
			}
			if (j === 0 && chunks.length > regionSize) {
				const lastChunk = chunks[chunks.length - regionSize];
				const downMostCell = lastChunk[(DIM - 1) * DIM + i];
				if (downMostCell && downMostCell.options) {
					const valid = downMostCell.options.flatMap(
						(opt) => rules[opt][2]
					); // RIGHT rule
					checkValid(options, valid);
				}
			...
			if (options.length === 0) {
				console.log("Ошибка в ячейке:", i, j);
				cell.error = true;
				resetGrid();
				// restoreGridState();
				if (i <= 1 || j <= 1) {
					console.log(
						"ошибка в начале создания. возвращаемся на чанк назад"
					);
...
sketch.draw = () => {
	background(100);
	noSmooth();
	// Отрисовываем все сохраненные чанки
	chunks.forEach((chunk, index) => {
		const x = index % regionSize;
		const y = Math.floor(index / regionSize);
		drawGrid(DIM, chunk, tiles, regularMapTilesIndex, x, y);
	});
	// Отрисовываем текущий grid (если он еще не добавлен в chunks)
	if (!isGridFullyCollapsed()) {
		const x = chunks.length % regionSize;
		const y = Math.floor(chunks.length / regionSize);
		drawGrid(DIM, grid, tiles, regularMapTilesIndex, x, y);
	}
	// Новая функция для применения ограничений от предыдущего чанка
	const applyBorderConstraints = () => {
		if (chunks.length === 0) return;

		const lastChunk = chunks[chunks.length - 1];

		for (let j = 0; j < DIM; j++) {
			const rightmostCell = lastChunk[(j + 1) * DIM - 1]; // Последняя ячейка в строке предыдущего чанка
			const currentCell = grid[j * DIM]; // Первая ячейка в строке текущего чанка

			// Применяем ограничения: варианты currentCell должны быть совместимы с rightmostCell
			const validOptions = rightmostCell.options.flatMap(
				(opt) => rules[opt][1]
			); // RIGHT rule
	...
	if (isGridFullyCollapsed()) {
		chunks.push([...grid]); // Сохраняем текущий grid в chunks
		resetGrid();
		applyBorderConstraints(); // Применяем ограничения от предыдущего чанка
		return;
	}
